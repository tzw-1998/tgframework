...
import com.tzw.tgframework.autoconfigure.DevToolsProperties;
import com.tzw.tgframework.autoconfigure.OptionalLiveReloadServer;
import com.tzw.tgframework.autoconfigure.RemoteDevToolsProperties;
import com.tzw.tgframework.autoconfigure.TriggerFileFilter;
import com.tzw.tgframework.classpath.ClassPathChangedEvent;
import com.tzw.tgframework.classpath.ClassPathFileSystemWatcher;
import com.tzw.tgframework.classpath.ClassPathRestartStrategy;
import com.tzw.tgframework.classpath.PatternClassPathRestartStrategy;
import com.tzw.tgframework.filewatch.FileSystemWatcher;
import com.tzw.tgframework.filewatch.FileSystemWatcherFactory;
import com.tzw.tgframework.livereload.LiveReloadServer;
import com.tzw.tgframework.restart.DefaultRestartInitializer;
import com.tzw.tgframework.restart.RestartScope;
...
import com.tzw.tgframework.autoconfigure.DevToolsProperties.Restart;
import com.tzw.tgframework.autoconfigure.RemoteDevToolsProperties.Proxy;
...
public class RemoteClientConfiguration implements InitializingBean {
	 ...
	 */
	@Configuration(proxyBeanMethods = false)
	@ConditionalOnProperty(prefix = "tgframework.livereload", name = "enabled", matchIfMissing = true)
	 ...
	 */
	@Configuration(proxyBeanMethods = false)
	@ConditionalOnProperty(prefix = "tgframework.remote.restart", name = "enabled", matchIfMissing = true)
	static class RemoteRestartClientConfiguration {
		...
		ClassPathChangeUploader classPathChangeUploader(ClientHttpRequestFactory requestFactory,
			...
			for (int i = 0;i<remoteUrls.length;i++){
				String url = remoteUrls[i] + this.properties.getRemote().getContextPath() + "/restart";
				remoteUrls[i] = url;
			}
			return new ClassPathChangeUploader(remoteUrls, requestFactory);
		}
